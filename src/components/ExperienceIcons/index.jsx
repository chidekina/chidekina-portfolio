const ExperienceIcons = ({ list, onClick, step }) => {
    return (
        <>
            <div className="block md:hidden w-full max-w-2xl z-50 flex justify-center mb-8">
                <div className="relative flex items-center justify-between px-4 w-full">
                    <div className="absolute top-1/2 left-4 right-4 h-0.5 bg-tertiary transform -translate-y-1/2 z-0"></div>
                    {list.map((item, index) => (
                        <div key={index} className="relative z-10">
                            <button
                                onClick={() => onClick(index)}
                                className={`flex items-center justify-center w-10 h-10 rounded-full border-2 transition-all duration-300 hover:scale-110 ${index === step
            ...
            <div className="hidden md:block fixed bottom-30 left-1/2 transform -translate-x-1/2 w-full max-w-2xl z-50">
